...
export function BetaGate({ status }: { status: string }) {
  ...
  const request=async()=>{setBusy(true);const response=await fetch("/api/beta/request",{method:"POST"});setRequested(response.ok);setNotice(response.ok?"Your request is with the Antiaging Labs team.":"Could not submit the request yet.");setBusy(false);};
