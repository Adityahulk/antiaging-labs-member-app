...
const initialMessages: Message[] = [
  { role: "assistant", text: "I can help with today’s protocol, your released results, meals, training, or trends already calculated from your health data. What would be most useful right now?" },
];
  ...
  return (
    <div className="chat-layout">
      <section className="chat-main">
        <div className="chat-topline"><div><span className="ai-orb">✦</span><span><strong>Antiaging Labs Guide</strong><small>Using Twin v{String(data?.twin?.version??"—")} · protocol v{String(data?.protocol?.version??"—")}</small></span></div><button type="button">•••</button></div>
      ...
      <aside className="chat-context">
        ...
        <div className="context-card"><div><span className="context-icon">◈</span><strong>Biological Twin</strong></div><p>{data?.twin?.domains.length??0} active domains · snapshot v{String(data?.twin?.version??"—")}</p><a href="/twin">View Twin →</a></div>
        <div className="context-card"><div><span className="context-icon">✓</span><strong>Protocol v{String(data?.protocol?.version??"—")}</strong></div><p>{data?.protocol?.actions.length??0} current actions</p><a href="/protocol">Open plan →</a></div>
