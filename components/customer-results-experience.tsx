...
export function CustomerResultsExperience() {
  ...
  return <>
    ...
    <section className="learning-memory-hero"><div><span className="card-kicker">YOUR RESPONSE MEMORY</span><h2>The app gets more valuable each time it learns from you.</h2><p>Every focused test becomes a permanent, traceable memory—what you tried, how consistently you did it, what changed, and whether it earned a place in your plan.</p></div><dl><div><dt>Learnings recorded</dt><dd>{assessments.length}</dd></div><div><dt>Useful signals</dt><dd>{conclusive}</dd></div><div><dt>Still uncertain</dt><dd>{uncertain}</dd></div><div><dt>Plan decisions</dt><dd>{planDecisions}</dd></div></dl></section>
    {!assessments.length ? <section className="paper-card result-readiness"><div><span className="card-kicker">YOUR FIRST PERSONAL LEARNING</span><h2>{activeExperiment || activeIntervention ? "Your learning isn’t ready yet—and that is expected." : "Your operating manual begins with one clean response test."}</h2><p>{activeExperiment || activeIntervention ? "Keep following the assigned days. Your Twin needs enough recorded routines and usable outcome data before it can separate your response from ordinary variation." : "Health trends can show movement, but a linked before-and-after response test is how the app learns whether one change worked for you."}</p><Link className="primary-button" href="/plan">{activeExperiment || activeIntervention ? "Complete today’s action" : "Choose a focused response test"}<span>→</span></Link></div><div className="result-progress"><strong>{recorded}<small>/{planned || "—"}</small></strong><span>useful days recorded</span><div><i style={{ width: planned ? `${Math.round(recorded / planned * 100)}%` : "0%" }}/></div><p>{planned ? `${Math.max(planned - recorded, 0)} days remain in this cycle` : "A schedule has not been created yet"}</p></div></section> : <section className="result-section-head"><span className="card-kicker">WHAT WE HAVE LEARNED ABOUT YOU</span><h2>Responses connected to a specific change</h2><p>These are the learnings strong enough to support a keep, adjust, or stop decision. Open any card to inspect uncertainty and context.</p></section>}
    ...
    <section className="paper-card health-trends"><div className="section-head compact"><div><span className="card-kicker">CHANGES WORTH NOTICING</span><h2>Movement that may shape the next question</h2><p>These comparisons are not yet linked to one specific change. They can guide what to test next, but they do not prove why anything moved.</p></div><span className="eta-chip">{trends.length} trends</span></div>{trends.length ? <><div className="trend-list">{trends.slice(0, 3).map((trend) => <TrendRow key={String(trend.id)} item={trend} />)}</div>{trends.length > 3 ? <details className="more-trends"><summary>See {trends.length - 3} more health trends</summary><div className="trend-list">{trends.slice(3).map((trend) => <TrendRow key={String(trend.id)} item={trend} />)}</div></details> : null}</> : <p>No reliable longitudinal comparisons are available yet.</p>}</section>

    <section className="learning-loop-timeline paper-card"><span className="card-kicker">HOW YOUR OPERATING MANUAL COMPOUNDS</span><div><span className="complete"><i>1</i><strong>Baseline built</strong><small>Your starting state</small></span><b>→</b><span className={activeExperiment || activeIntervention || assessments.length ? "complete" : ""}><i>2</i><strong>Response tested</strong><small>One focused change</small></span><b>→</b><span className={assessments.length ? "complete" : ""}><i>3</i><strong>Learning recorded</strong><small>What happened for you</small></span><b>→</b><span className={planDecisions ? "complete" : ""}><i>4</i><strong>Plan updated</strong><small>Keep, adjust or stop</small></span><b>→</b><span><i>5</i><strong>Retest</strong><small>Confirm longer-term movement</small></span></div></section>
