...
export function CustomerResultsExperience() {
  ...
  return <>
    ...
    <section className="learning-memory-hero"><div><span className="card-kicker">SO FAR</span><h2>A running record of what changed for you.</h2><p>Each focused test keeps what you tried, how consistently you did it, what moved, and whether it earned a place in the plan.</p></div><dl><div><dt>Learnings</dt><dd>{assessments.length}</dd></div><div><dt>Useful signals</dt><dd>{conclusive}</dd></div><div><dt>Still uncertain</dt><dd>{uncertain}</dd></div><div><dt>Plan decisions</dt><dd>{planDecisions}</dd></div></dl></section>
    {!assessments.length ? <section className="paper-card result-readiness"><div><span className="card-kicker">FIRST LEARNING</span><h2>{activeExperiment || activeIntervention ? "This learning isn’t ready yet—and that is expected." : "The first result starts with one clean comparison."}</h2><p>{activeExperiment || activeIntervention ? "Keep following the assigned days. Enough recorded routines and usable outcome data are needed before a personal response can be separated from ordinary variation." : "Health trends can show movement, but a linked before-and-after test is how the app learns whether one change worked for you."}</p><Link className="primary-button" href="/plan">{activeExperiment || activeIntervention ? "Complete today’s action" : "Choose a focus"}<span>→</span></Link></div><div className="result-progress"><strong>{recorded}<small>/{planned || "—"}</small></strong><span>useful days recorded</span><div><i style={{ width: planned ? `${Math.round(recorded / planned * 100)}%` : "0%" }}/></div><p>{planned ? `${Math.max(planned - recorded, 0)} days remain in this cycle` : "A schedule has not been created yet"}</p></div></section> : <section className="result-section-head"><span className="card-kicker">WHAT WE HAVE LEARNED ABOUT YOU</span><h2>Responses connected to a specific change</h2><p>These are the learnings strong enough to support a keep, adjust, or stop decision. Open any card to inspect uncertainty and context.</p></section>}
