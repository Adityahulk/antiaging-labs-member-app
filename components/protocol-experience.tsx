...
export function ProtocolExperience() {
  ...
  return <>
    ...
    <section className="protocol-hero focused-protocol" aria-labelledby="active-intervention-title"><div className="protocol-strategy"><span className="card-kicker">CURRENT FOCUS · DAY {dayNumber}{totalDays ? ` OF ${totalDays}` : ""}</span><h2 id="active-intervention-title">{title}</h2><p>{hypothesis}</p></div><div className="cycle-progress"><div className="large-ring" style={{ background: `conic-gradient(#d07d5a ${totalDays ? Math.round(completedDays / totalDays * 100) : 0}%,rgba(255,255,255,.1) 0)` }}><div><strong>{completedDays}/{totalDays || "—"}</strong><span>USEFUL DAYS</span></div></div><div><strong>{readable(intervention?.status ?? active?.status ?? "active")}</strong><span>Learning appears after enough usable days.</span><a href="/learnings">Check learning readiness →</a></div></div></section>

    {nextPeriod || intervention?.status === "active" ? <section className="experiment-today-grid">
      <article className="paper-card experiment-action-card"><div className="experiment-action-head"><div><span className="card-kicker">TODAY · {isControl ? "USUAL ROUTINE" : "FOCUSED CHANGE"}</span><h2>{isControl ? `Keep it usual: ${instruction}` : instruction}</h2></div><span className={isControl ? "control-badge" : "intervention-badge"}>{isControl ? "A" : "B"}</span></div><p>{isControl ? "Keep the rest of your routine as normal." : "Follow this one change. Keep everything else consistent."}</p><label className="context-note"><span>Anything unusual that could affect today? <small>optional</small></span><input value={note} onChange={(event) => setNote(event.target.value)} placeholder="Travel, illness, alcohol, poor sleep, hard training…" /></label><div className="adherence-actions"><button disabled={Boolean(busy)} onClick={() => void checkIn(0)} type="button">Couldn’t do it</button><button disabled={Boolean(busy)} onClick={() => void checkIn(.5)} type="button">Partly done</button><Button trailing={null} disabled={Boolean(busy)} onClick={() => void checkIn(1)}>{busy ? "Saving…" : "Done as planned"}</Button></div></article>
      <aside className="paper-card experiment-why-card"><span className="card-kicker">WHAT YOUR TWIN IS LEARNING</span><h3>{outcome}</h3><dl><div><dt>Broader goal</dt><dd>{data?.member?.primaryGoal || "Not captured"}</dd></div><div><dt>The question</dt><dd>{hypothesis}</dd></div><div><dt>Review point</dt><dd>{dateLabel(intervention?.reviewAt ?? active?.endAt)}</dd></div></dl></aside>
    </section> : null}

    {periods.length ? <section className="paper-card experiment-calendar"><div className="section-head compact"><div><span className="card-kicker">YOUR CYCLE</span><h2>A vs B days</h2></div><span className="eta-chip">{completed.length}/{periods.length} recorded</span></div><div className="period-track">{periods.map((period) => <div className={period.completed ? "complete" : period.id === nextPeriod?.id ? "today" : ""} key={String(period.id)}><span>{new Date(String(period.day)).getUTCDate()}</span><i>{String(period.arm)}</i><small>{period.completed ? "Done" : period.id === nextPeriod?.id ? "Today" : "Next"}</small></div>)}</div><p className="calendar-key"><span><i className="a"/>A = your usual routine</span><span><i className="b"/>B = the experimental change</span></p></section> : null}

    <section className="experiment-support-grid"><article className="paper-card"><span className="card-kicker">SAFETY</span><h2>{safetyCopy.title}</h2><p>{safetyCopy.body}</p><a href="/ask?topic=safety">Ask about safety →</a></article><article className="paper-card"><span className="card-kicker">DNA IN THIS EXPERIMENT</span><h2>{reviewedDna ? `${reviewedDna} reviewed findings available` : "No reviewed DNA link used"}</h2><p>{reviewedDna ? "Judged by observed response, not genotype." : "DNA is optional for this hypothesis."}</p><a href="/genetics">Inspect genetics →</a></article></section>
