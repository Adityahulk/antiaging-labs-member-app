...
export function ProtocolExperience() {
  ...
  return <>
    ...
    {intervention ? <section className="protocol-controls"><span>You remain in control of this response test.</span><div>{intervention.status === "active" ? <button disabled={Boolean(busy)} onClick={() => void mutate("pause")}>Pause</button> : intervention.status === "paused" ? <button disabled={Boolean(busy)} onClick={() => void mutate("resume")}>Resume</button> : null}<button disabled={Boolean(busy)} onClick={() => void mutate("stop")}>Stop this test</button><a href="/ask?topic=active-experiment">Ask why this →</a></div></section> : <p className="legacy-cycle-note">This cycle began in an earlier version of the app. Your check-ins remain valid; new cycles will also include pause and stop controls.</p>}
  </>;
}
