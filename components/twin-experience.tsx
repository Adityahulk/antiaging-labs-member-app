...
export function TwinExperience() {
  ...
  return <>
    <section className="twin-story-grid">
      <article><span className="card-kicker">01 · WHAT WE MEASURED</span><strong>{measuredCount} of {domains.length} domains have a current signal</strong><p>Bloodwork, wearable data, intake and reviewed DNA keep their original source and date.</p><a href="/data">Manage my data →</a></article>
      <article className="priority"><span className="card-kicker">02 · WHAT APPEARS IMPORTANT</span><strong>{String(priorityDomain?.label ?? "A priority is still being built")}</strong><p>{String(data?.twin?.summary ?? "Your Twin will name the clearest priority once enough reliable data is available.")}</p></article>
      <article><span className="card-kicker">03 · WHAT WE ARE TESTING</span><strong>{String(activeFocus?.title ?? "No current response test")}</strong><p>{String(activeFocus?.hypothesis ?? "Your next focus will be selected from your goals, current state, safety and measurable outcomes.")}</p><a href="/plan">Open my plan →</a></article>
      <article><span className="card-kicker">04 · WHAT WE LEARNED</span><strong>{learnedCount ? `${learnedCount} personal ${learnedCount === 1 ? "learning" : "learnings"} recorded` : "Your response memory starts with the first completed test"}</strong><p>Useful, uncertain and repeated responses stay attached to the change that produced them.</p><a href="/learnings">Open my response memory →</a></article>
    </section>
