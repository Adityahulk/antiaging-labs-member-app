...
export function TwinExperience() {
  ...
  if (!selected) return <div className="paper-card empty-state"><h2>Building your Twin</h2><p>Add a wearable, lab result, or genetics file. Domains stay unknown until there is enough evidence.</p><button onClick={() => void recompute()} className="primary-button" type="button">Build now →</button></div>;
  ...
  return <>
    <section className="twin-story-grid">
      <article><span className="card-kicker">01 · MEASURED</span><strong>{measuredCount} of {domains.length} domains have a current signal</strong><a href="/data">Manage my data →</a></article>
      <article className="priority"><span className="card-kicker">02 · STANDS OUT</span><strong>{String(priorityDomain?.label ?? "A priority is still being built")}</strong><p>{String(data?.twin?.summary ?? "Named once enough reliable data is available.")}</p></article>
      <article><span className="card-kicker">03 · TESTING</span><strong>{String(activeFocus?.title ?? "No current focus")}</strong><p>{String(activeFocus?.hypothesis ?? "Chosen from your goal, current state, safety and measurable outcomes.")}</p><a href="/plan">Open my plan →</a></article>
      <article><span className="card-kicker">04 · LEARNED</span><strong>{learnedCount ? `${learnedCount} personal ${learnedCount === 1 ? "learning" : "learnings"} recorded` : "Starts after the first completed test"}</strong><a href="/learnings">Open learnings →</a></article>
    </section>
    ...
    <section className="domain-overview paper-card"><div className="section-head compact"><div><span className="card-kicker">DOMAINS</span><h2>Measured, possible, or unknown</h2></div></div><div>{domains.map((domain) => { const detail = (domain.evidenceJson ?? {}) as Evidence; const known = isMeasured(detail); const domainReadiness = readinessFor(detail, domain.confidence ?? 0); return <button onClick={() => { setSelectedCode(String(domain.domainCode)); window.scrollTo({ top: 0, behavior: "smooth" }); }} className={domain.domainCode === selected.domainCode ? "active" : ""} type="button" key={String(domain.domainCode)}><span className={`status-dot ${known ? "known" : "unknown"}`}/><strong>{String(domain.label)}</strong><small>{known ? String(domain.stateLabel) : `${domainReadiness}% data ready`}</small><em>{String(domain.keyMetric ?? "No key signal yet")} {domain.keyValue !== null && domain.keyValue !== undefined ? `· ${String(domain.keyValue)} ${String(domain.keyUnit ?? "")}` : ""}</em><i>→</i></button>; })}</div></section>

    <section className="paper-card fusion-stack inherited-summary">
      <div className="section-head compact"><div><span className="card-kicker">DNA · {String(selected.label).toUpperCase()}</span><h2>{inherited.length ? "Linked inherited clues" : "No DNA clue linked here yet"}</h2><p className="fusion-note">DNA generates hypotheses; it never increases a domain score or creates an action on its own.</p></div><span className="eta-chip">{inherited.length} linked</span></div>
      {inherited.length ? <div className="fusion-layers">{inherited.slice(0, 2).map((item, index) => <article className={item.reviewStatus === "reviewed" ? "available" : "pending"} key={item.id ?? `${item.gene}-${item.rsid}-${index}`}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{item.gene} · {item.title}</strong><small>{item.reviewStatus === "reviewed" ? "reviewed context" : "review pending"} · {item.evidenceLabel}</small><p>{item.hypothesis}</p><p><strong>{phenotypeLabel(item.phenotype?.status)}</strong></p><p><strong>What it changes:</strong> {item.whatThisChanges}</p></div><i/></article>)}</div> : <div className="empty-state"><p>DNA stays context until measurements support it. It never creates an action by itself.</p></div>}
