...
export const memberGoals = sqliteTable("member_goals", {
  ...
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull(),
}, (table) => [index("idx_member_goals_member_status").on(table.memberId, table.status), index("idx_member_goals_member_updated").on(table.memberId, table.updatedAt)]);

export const safetyDecisions = sqliteTable("safety_decisions", {
  id: text("id").primaryKey(),
  memberId: text("member_id").notNull(),
  entityType: text("entity_type").notNull(),
  entityId: text("entity_id").notNull(),
  ...
  createdAt: text("created_at").notNull(),
}, (table) => [index("idx_safety_member_status").on(table.memberId, table.status), index("idx_safety_entity_time").on(table.entityType, table.entityId, table.decidedAt)]);

export const priorityAssessments = sqliteTable("priority_assessments", {
  id: text("id").primaryKey(),
  memberId: text("member_id").notNull(),
  goalId: text("goal_id"),
  twinSnapshotId: text("twin_snapshot_id"),
  safetyDecisionId: text("safety_decision_id"),
  recommendedCandidateId: text("recommended_candidate_id"),
  status: text("status").notNull(),
  engineVersion: text("engine_version").notNull(),
  evidenceVersion: text("evidence_version").notNull(),
  inputSnapshotJson: text("input_snapshot_json").notNull(),
  createdAt: text("created_at").notNull(),
}, (table) => [index("idx_priority_assessments_member_time").on(table.memberId, table.createdAt), index("idx_priority_assessments_status").on(table.status, table.createdAt)]);

export const priorityCandidates = sqliteTable("priority_candidates", {
  id: text("id").primaryKey(),
  ...
  domainCode: text("domain_code").notNull(),
  title: text("title").notNull(),
  userImportance: real("user_importance").notNull(),
  actionability: real("actionability").notNull(),
  measurementReadiness: real("measurement_readiness").notNull(),
  evidenceConfidence: real("evidence_confidence").notNull(),
  timeToSignal: real("time_to_signal").notNull(),
  burden: real("burden").notNull(),
  riskPenalty: real("risk_penalty").notNull(),
  geneticsModifier: real("genetics_modifier").notNull(),
  finalScore: real("final_score").notNull(),
  rank: integer("rank").notNull(),
  rationaleJson: text("rationale_json").notNull(),
  evidenceRefsJson: text("evidence_refs_json").notNull(),
  missingJson: text("missing_json").notNull(),
  experimentTemplatesJson: text("experiment_templates_json").notNull(),
  createdAt: text("created_at").notNull(),
}, (table) => [uniqueIndex("idx_priority_candidate_assessment_code").on(table.assessmentId, table.candidateCode), uniqueIndex("idx_priority_candidate_assessment_rank").on(table.assessmentId, table.rank), index("idx_priority_candidates_member_score").on(table.memberId, table.finalScore)]);

export const interventionEpisodes = sqliteTable("intervention_episodes", {
  id: text("id").primaryKey(),
  memberId: text("member_id").notNull(),
  goalId: text("goal_id"),
  priorityCandidateId: text("priority_candidate_id"),
  safetyDecisionId: text("safety_decision_id").notNull(),
  sourceExperimentId: text("source_experiment_id"),
  sourceProtocolActionId: integer("source_protocol_action_id"),
  title: text("title").notNull(),
  category: text("category").notNull(),
  hypothesis: text("hypothesis").notNull(),
  exactInstruction: text("exact_instruction").notNull(),
  doseOrDuration: text("dose_or_duration"),
  frequency: text("frequency").notNull(),
  primaryOutcomeCode: text("primary_outcome_code").notNull(),
  outcomeDirection: text("outcome_direction").notNull(),
  outcomeUnit: text("outcome_unit").notNull(),
  targetMin: real("target_min"),
  targetMax: real("target_max"),
  minimumBaselineDays: integer("minimum_baseline_days").notNull(),
  minimumComparisonDays: integer("minimum_comparison_days").notNull(),
  ...
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull(),
}, (table) => [index("idx_interventions_member_status_review").on(table.memberId, table.status, table.reviewAt), index("idx_interventions_candidate").on(table.priorityCandidateId), index("idx_interventions_outcome").on(table.primaryOutcomeCode, table.status)]);

export const interventionExposures = sqliteTable("intervention_exposures", {
  id: text("id").primaryKey(),
  interventionEpisodeId: text("intervention_episode_id").notNull(),
  memberId: text("member_id").notNull(),
  scheduledAt: text("scheduled_at").notNull(),
  occurredAt: text("occurred_at"),
  plannedValue: text("planned_value"),
  actualValue: text("actual_value"),
  adherence: real("adherence").notNull(),
  completed: integer("completed", { mode: "boolean" }).notNull(),
  subjectiveResponse: real("subjective_response"),
  adverseEffect: integer("adverse_effect", { mode: "boolean" }).notNull(),
  note: text("note").notNull(),
  source: text("source").notNull(),
  createdAt: text("created_at").notNull(),
}, (table) => [uniqueIndex("idx_exposure_episode_schedule").on(table.interventionEpisodeId, table.scheduledAt), index("idx_exposures_member_time").on(table.memberId, table.scheduledAt), index("idx_exposures_episode_completed").on(table.interventionEpisodeId, table.completed)]);

export const contextEvents = sqliteTable("context_events", {
  id: text("id").primaryKey(),
  memberId: text("member_id").notNull(),
  interventionEpisodeId: text("intervention_episode_id"),
  ...
  source: text("source").notNull(),
  createdAt: text("created_at").notNull(),
}, (table) => [index("idx_context_member_time").on(table.memberId, table.occurredAt), index("idx_context_episode_time").on(table.interventionEpisodeId, table.occurredAt), index("idx_context_type_time").on(table.type, table.occurredAt)]);

export const responseAssessments = sqliteTable("response_assessments", {
  id: text("id").primaryKey(),
  memberId: text("member_id").notNull(),
  interventionEpisodeId: text("intervention_episode_id").notNull(),
  version: integer("version").notNull(),
  ...
  comparisonStart: text("comparison_start").notNull(),
  comparisonEnd: text("comparison_end").notNull(),
  baselineValue: real("baseline_value"),
  comparisonValue: real("comparison_value"),
  absoluteChange: real("absolute_change"),
  percentChange: real("percent_change"),
  effectEstimate: real("effect_estimate"),
  lowerBound: real("lower_bound"),
  upperBound: real("upper_bound"),
  dataQuality: real("data_quality").notNull(),
  adherence: real("adherence").notNull(),
  baselineUsableDays: integer("baseline_usable_days").notNull(),
  comparisonUsableDays: integer("comparison_usable_days").notNull(),
  confoundersJson: text("confounders_json").notNull(),
  attributionGrade: text("attribution_grade").notNull(),
  conclusion: text("conclusion").notNull(),
  recommendedDecision: text("recommended_decision").notNull(),
  insufficiencyReasonsJson: text("insufficiency_reasons_json").notNull(),
  sourceRefsJson: text("source_refs_json").notNull(),
  analysisVersion: text("analysis_version").notNull(),
  computedAt: text("computed_at").notNull(),
  reviewedAt: text("reviewed_at"),
}, (table) => [uniqueIndex("idx_response_assessment_episode_version").on(table.interventionEpisodeId, table.version), index("idx_response_assessments_member_time").on(table.memberId, table.computedAt), index("idx_response_assessments_status_time").on(table.status, table.computedAt), index("idx_response_assessments_outcome_time").on(table.primaryOutcomeCode, table.computedAt)]);

export const geneticHypothesisLinks = sqliteTable("genetic_hypothesis_links", {
  id: text("id").primaryKey(),
  memberId: text("member_id").notNull(),
  interventionEpisodeId: text("intervention_episode_id").notNull(),
  genomicInterpretationId: text("genomic_interpretation_id").notNull(),
  influence: text("influence").notNull(),
  predictedRelationship: text("predicted_relationship").notNull(),
  observedStatus: text("observed_status").notNull(),
  evidenceVersion: text("evidence_version").notNull(),
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull(),
}, (table) => [uniqueIndex("idx_genetic_link_intervention_interpretation").on(table.interventionEpisodeId, table.genomicInterpretationId), index("idx_genetic_links_member_status").on(table.memberId, table.observedStatus), index("idx_genetic_links_interpretation").on(table.genomicInterpretationId)]);
...
export const productEvents = sqliteTable("product_events", {
  ...
  occurredAt: text("occurred_at").notNull(),
}, (table) => [index("idx_product_events_name_time").on(table.eventName, table.occurredAt), index("idx_product_events_member_time").on(table.memberId, table.occurredAt), index("idx_product_events_cohort_name").on(table.cohortCode, table.eventName)]);
