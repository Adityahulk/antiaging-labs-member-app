...
| AI model | Deterministic fallback, grounded gateway calls, audit envelopes, safety classification and review queue exist | Choose a model gateway/provider; configure `AI_GATEWAY_URL` and optional `AI_GATEWAY_TOKEN`; enforce no-retention terms, model allowlist, budgets, red-team suite and clinical review targets |
