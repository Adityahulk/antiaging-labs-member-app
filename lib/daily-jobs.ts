...
export async function runDailyJobs() {
  ...
  const phase3=await runPhase3Jobs(); await db.prepare("INSERT INTO webhook_events (id,provider,event_type,payload_json,status,received_at,processed_at) VALUES (?,'system','daily_loop',?,'processed',?,?)").bind(id("jobrun"),JSON.stringify({results,phase3}),nowIso(),nowIso()).run(); return {processed:results.length,results,phase3};
}
